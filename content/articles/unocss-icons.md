...
description: "在 Vue 项目里，图标通常有三条路：组件库内置图标、<svg> 内联、或 Sprite 组件。它们都能用，但每种方式都多一层心智负担——要 import 组件、记 props，或在模板和脚本之间来回切换。"
date: 2026-06-12
tags: ["UnoCSS", "图标", "Icon"]
juejin: https://juejin.cn/post/7650093760348848147
slug: unocss-icons
---

# 从 uno.config.ts 看懂 UnoCSS 图标方案

**Meta description:** 从项目的 `uno.config.ts` 出发，详解 UnoCSS presetIcons 如何把本地 SVG 编译成 `i-custom-*` 原子类，并配合 VS Code Iconify 扩展实现开发体验闭环。
...
UnoCSS 的 **presetIcons** 走另一条路：**图标就是 CSS 类名**。写 `class="i-custom-confirm-circle-filled text-success"`，UnoCSS 在构建时把 SVG 转成背景图工具类，颜色、尺寸继续用现有的原子类控制。本文从本仓库的 `uno.config.ts` 出发，拆解这套方案如何落地。
...
### 3. 输出：presetIcons 生成 CSS 工具类

`presetIcons` 基于 [Iconify](https://iconify.design/) 生态，把 SVG 转成 **mask 或 background** 形式的单类图标。配合 `extraProperties`：
...
```vue
<template>
  <span class="i-custom-my-icon text-primary text-16" aria-hidden="true" />
  <span>操作成功</span>
</template>
```
...
1. 确认 `src/assets/svg/xxx.svg` 存在，且文件名与类名一致（kebab-case）
2. 确认类名字符串完整出现在源码中（UnoCSS 静态扫描，动态拼接类名可能被漏扫）
3. 开发环境下检查 `.vscode/icons.json` 是否已生成
...
1. 在 `src/assets/svg/` 添加你的业务图标，跑一遍 `pnpm dev` 验证补全
2. 阅读 [UnoCSS presetIcons 文档](https://unocss.dev/presets/icons) 了解 `scale`、`mode`、`customizations` 等高级选项
3. 在团队规范中明确：**何时用 `i-custom-*`，何时用 `@element-plus/icons-vue`**
